class KFBossSquadS extends Mutator
    config(KFBossSquadS);
...
function PostBeginPlay()
{
	...
	Class'UTServerAdmin'.Default.QueryHandlerClasses[Class'UTServerAdmin'.Default.QueryHandlerClasses.Length] = "KFBossSquadS.SumoSPxWebQueryDefaults";
...
function InitSpawnFx()
{
    ...
    SpawnFxClass = Class<SumoSpawnFx>(DynamicLoadObject(Class'KFBossSquadS'.Default.SpawnFx,Class'Class'));
	if( SpawnFxClass==None )
	{
		log("FATAL ERROR: SpawnFx '"$Class'KFBossSquadS'.Default.SpawnFx$"' not found");
		log("Check KFBossSquadS.ini -> [KFBossSquadS.KFBossSquadS] -> SpawnFx"); 
		ConsoleCommand("exit");
	}
...
function InitMonsters()
{
    ...
    log("*******************************************************************");
	MonsterNames = GetPerObjectNames("KFBossSquadS", string(class'SumoSPMonster'.Name));
...
function Mutate( string MutateString, PlayerController Sender )
{
    ...
    if( Sender.PlayerReplicationInfo.bAdmin || Sender.PlayerReplicationInfo.bSilentAdmin || Viewport(Sender.Player)!=None )
    {
        ...
        {
            Sender.ClientMessage("=== KFBossSquadS commands ===");
            ...
            Sender.ClientMessage("MUTATE RSPD: Respawn all deads ");
        }
        ...
        else if(MutateString ~= "RSPD")
        {
            if(ResurrectPlayers() > 0)
            {
                if(!Sender.PlayerReplicationInfo.bSilentAdmin)
                {
                    BroadcastText(("%g" $ Sender.PlayerReplicationInfo.PlayerName) $ " %whas %rresurrected %wall the deads.");
                }
                else
                {
                    SendMessage(Sender, "%gYou %whave %rresurrected %wall the players.");
                }
            }
            else
            {
                SendMessage(Sender, "%gAll %wthe players are still %ralive%w.");
            }
        }
    }
    if( NextMutator!=None )
...
function SendMessage(PlayerController Player, coerce string Message)
{
    ...
    Player.TeamMessage(none, Message, 'KFBossSquadS');
}
...
function int ResurrectPlayers()
{
    ...
    bWaveState = GT.bWaveInProgress;
    GT.Disable('Timer');
    GT.bWaveInProgress = false;
    ...
    GT.bWaveInProgress = bWaveState;
    GT.Enable('Timer');
    return AliveCount;
}

function SetTraderTime(int Time)
{
    if(!GT.bWaveInProgress && GT.WaveNum <= GT.FinalWave)
    {
        GT.WaveCountDown = Clamp(Time, 1, 1024);
        if(KFGameReplicationInfo(GT.GameReplicationInfo) != none)
        {
            KFGameReplicationInfo(GT.GameReplicationInfo).WaveNumber = byte(GT.WaveNum);
        }
    }
}
...
/* function SetPlayersNames()
{
    ...
    for(C = Level.ControllerList;C != none;C = C.nextController)
    {
        if(C.IsA('PlayerController') && C.PlayerReplicationInfo != none)
        {
            C.PlayerReplicationInfo.PlayerName = PlayerNames[Rand(PlayerNames.Length)];
        }
    }
} */
...
static function FillPlayInfo(PlayInfo PlayInfo)
{
    ...
    PlayInfo.AddSetting(default.GroupName,"BeginningTime","Start time of the match",1, 0, "Text", "3;1:999");
    PlayInfo.AddSetting(default.GroupName,"Maxslot","Set Max player slots",1, 0, "Text", "3;1:50");
    PlayInfo.AddSetting(default.GroupName,"NumWaves","Set game waves",1, 0, "Text", "2;1:99");
}
...
static event string GetDescriptionText(string PropName)
{
    switch (PropName)
    {
        ...
        case "BeginningTime":          return "It's the time during which the match will be started.";
        case "Maxslot":                return "Max players slots.";
        case "NumWaves":                return "Number of waves.";
    }
...
defaultproperties
{
     ...
     GroupName="KF-BossSquadS"
     FriendlyName="KFBossSquadS"
     Description="BossSquad Sandbox mode, now with beginningTraderTime, custom gameWaves and player slots, modified by GutturalSickness"
