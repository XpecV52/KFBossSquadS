...
function BeginPlay()
{
    ...
    if (Class'KFBossSquadS'.Default.bEnableLifeBoost)
		PawnOwner.Health = 500;
    if (Class'KFBossSquadS'.Default.bEnableGodMode)
...
function Timer()
{
	if( !bHasInit )
	{
        ...
        SetTimer(Class'KFBossSquadS'.Default.BonusStageTime+15,false);
		return;
	}
	Disable('Tick');
	SetTimer(0.f,false);
...
function Tick( float Delta )
{
	...
	if( PlayerOwner!=None && Class'KFBossSquadS'.Default.bEnableGodMode)
		PlayerOwner.bGodMode = true;

	if( PawnOwner!=None && Class'KFBossSquadS'.Default.bEnableLifeBoost)
		PawnOwner.Health = 500;
}
...
function Destroyed()
{
	...
	if( PawnOwner!=None && PawnOwner.Health>100 && Class'KFBossSquadS'.Default.bEnableLifeBoost )
...
defaultproperties
{
     ...
     End Object
     Emitters(0)=SpriteEmitter'KFBossSquadS.BonusStageClient.SpriteEmitter0'
