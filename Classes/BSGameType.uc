...
var KFBossSquadS MonsterConfigMut;
...
function PreBeginPlay()
{
	...
	foreach AllActors(class'KFMod.KFRandomItemSpawn', KFRIS)
		if(KFRIS != None)
			KFRIS.Destroy();
...
{ 
    function BeginState()
    {
        ...
        // WaveCountDown = 30;// Modify this if we want to make it take long for zeds to spawn initially
        WaveCountDown = MonsterConfigMut.BeginningTime;// Modify this if we want to make it take long for zeds to spawn initially
        SetupPickups();
    }
            ...
            if( bBossSquadRequested )
            {
                ...
                else
                {
                    ...
                    if( Level.TimeSeconds > BonusStageStartTime-5 )
                    {
                        ...
                        for ( C = Level.ControllerList; C != None; C = C.NextController )
                            if( KFPlayerController(C) != None )
                                KFPlayerController(C).ClientSetMusic(Class'KFBossSquadS'.Default.BossTimeSong,MTRAN_FastFade);
                    }                
                }
            }
                ...
                if ( NumMonsters <= 0 )
                {
                    if ( !bBossSquadRequested )
                    {
                        ...
                        if( MidBossSetup.BossSquad != "" )
                        {
                            ...
                            BonusStageEndTime = NextMonsterTime + Class'KFBossSquadS'.Default.BonusStageTime;
                            ...
                            if( MidBossSetup.BonusStage )
							{
                                ...
                                TotalMaxMonsters = Class'KFBossSquadS'.Default.BonusStageNumMonsters;
							}
                            else
                            {
								SetMidBossSquad();
                            ...
                            if ( TotalMaxMonsters > 0 )
                            {
                                if( MidBossSetup.BonusStage && Class'KFBossSquadS'.Default.BonusStageSongPackage != "" )
                                {
                                ...
                                for ( C = Level.ControllerList; C != None; C = C.NextController )
                                {
                                    if( KFPlayerController(C) != None )
                                    {
                                        if( MidBossSetup.BonusStage )
                                        {
                                            ...
                                            if(Class'KFBossSquadS'.Default.BonusStageSong == "")
                                                KFPlayerController(C).NetStopMusic(5.0f);
                                            else
                                                KFPlayerController(C).ClientSetMusic(Class'KFBossSquadS'.Default.BonusStageSong,MTRAN_FastFade);
                                            KFPlayerController(C).ReceiveLocalizedMessage(Class'BossSquadMessage',1);
                                            C.Pawn.Spawn(Class'BonusStageClient',C.Pawn);
                                        }
                                        else
                                        {
                                            KFPlayerController(C).ClientSetMusic(Class'KFBossSquadS'.Default.BossTimeIntroSong,MTRAN_Instant);
...
function SetupWave()
{
    ...
    TotalMaxMonsters = Clamp(NewMaxMonsters,1,Class'KFBossSquadS'.Default.WaveMaxMonsters);
...
function MidBossAddSquad()
{
	if( NextNewSpawnSquad.Length > 0 )
	{
	    if (Class'KFBossSquadS'.Default.bEnableSpawnFx)
		{
        ...
        if( D != None && KFPlayerController(D) != None )
        {
            for ( C=Level.ControllerList; C!=None; C=C.NextController )
            {
                if( KFPlayerController(C) != None )
                {
                    ...
                    {
                        KFPlayerController(C).ReceiveLocalizedMessage(Class'BossSquadMessage',Class'KFBossSquadS'.Default.BonusStageCash,C.PlayerReplicationInfo,C.PlayerReplicationInfo);
                        C.Pawn.PlaySound( Class'CashPickup'.Default.PickupSound,SLOT_None,2);
                        C.PlayerReplicationInfo.Score += Class'KFBossSquadS'.Default.BonusStageCash;
                    }
                    else
                    {
                        KFPlayerController(C).ReceiveLocalizedMessage(Class'BossSquadMessage',Class'KFBossSquadS'.Default.BonusStageCash,C.PlayerReplicationInfo,D.PlayerReplicationInfo);
                    }
                }
            }
        }
...
event InitGame( string Options, out string Error )
{
	...
	{
        foreach DynamicActors(class'KFBossSquadS', MonsterConfigMut)
            break;
        if( MonsterConfigMut==None )
        {
			log("----------------------------------------------------------");
            Log("ERROR: InitGame(), KFBossSquadS mutator not loaded!");
			log("----------------------------------------------------------");
            return;
        }
	}
    ...
    if(KFGameLength != 3)
    {
        ...
        UpdateGameLength();
        // TimeBetweenWaves = MonsterConfigMut.TraderTime;
        // StartingCash = MonsterConfigMut.StartingCash;
        // MinRespawnCash = MonsterConfigMut.MinRespawnCash;
        FinalWave = MonsterConfigMut.NumWaves;
        InitialWave = 0;
    }
}
...
function ShowPathTo(PlayerController P, int TeamNum)
{
    ...
    {
        Spawn(class'CyanWhisp', P,, P.Pawn.Location);
    }
}
